...
//	{
//		L"asset/font/DotGothic16-Regular.ttf",
//	};
//}
//
////=============================================================================
...
//	// rect：領域指定
//	// options：テキストの整形
//	HRESULT DrawString(std::wstring wstr, D2D1_RECT_F rect, D2D1_DRAW_TEXT_OPTIONS options, bool shadow = false);
//
//	// 指定されたパスのフォントを読み込む
//	HRESULT FontLoader();
//
...
//private:
//
//	ComPtr <ID2D1Factory>			pD2DFactory = nullptr;		// Direct2Dリソース
//	ComPtr <ID2D1RenderTarget>		pRenderTarget = nullptr;	// Direct2Dレンダーターゲット
//	ComPtr <ID2D1SolidColorBrush>	pBrush = nullptr;			// Direct2Dブラシ設定
//	ComPtr <ID2D1SolidColorBrush>	pShadowBrush = nullptr;		// Direct2Dブラシ設定（影）
//	ComPtr <IDWriteFactory>		pDWriteFactory = nullptr;	// DirectWriteリソース
//	ComPtr <IDWriteTextFormat>		pTextFormat = nullptr;		// DirectWriteテキスト形式
//	ComPtr <IDWriteTextLayout>		pTextLayout = nullptr;		// DirectWriteテキスト書式
//	ComPtr <IDXGISurface>			pBackBuffer = nullptr;		// サーフェス情報
//
//	// フォントファイルリスト
//	std::vector<ComPtr<IDWriteFontFile>> pFontFileList;
//
//	// フォントデータ

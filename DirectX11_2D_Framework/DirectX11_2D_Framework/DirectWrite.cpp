//ComPtr <CustomFontCollectionLoader> pFontCollectionLoader = nullptr;
//
////=============================================================================
////		カスタムファイルローダー
////=============================================================================
...
//	if (FAILED(result)) { return result; }
//
//	HWND hwnd = GetActiveWindow();  // Or use any valid window handle
//	UINT dpi = GetDpiForWindow(hwnd);
//
//	// dpiの設定
//	FLOAT dpiX = static_cast<float>(dpi);  // Use dpiX for scaling
//
//	// レンダーターゲットの作成
//	D2D1_RENDER_TARGET_PROPERTIES props = D2D1::RenderTargetProperties(D2D1_RENDER_TARGET_TYPE_DEFAULT, D2D1::PixelFormat(DXGI_FORMAT_UNKNOWN, D2D1_ALPHA_MODE_PREMULTIPLIED), dpiX, dpiX);
//
...
//		pFontFileList.data(),
//		static_cast<int>(pFontFileList.size()),
//		&fontCollection
//	);
//
//	if (FAILED(result)) { return result; }
...
//{
//	return static_cast<int>(fontNamesList.size());
//}
//
//// フォント設定
//// 第1引数：フォントデータ構造体
...
//
//	const wchar_t* fileName = GetFontFileNameWithoutExtension(Setting.font.c_str());
...
//	delete[] fileName;
//	if (FAILED(result)) { 
//		const char* cstr = ConvertWCharToChar(Setting.font.c_str());
//		std::string log = cstr;
//		log = "テクスチャ読み込み失敗 :" + log;
...
//	HRESULT result = S_OK;
//
//	const wchar_t* fileName = GetFontFileNameWithoutExtension(fontname);
//	pDWriteFactory->CreateTextFormat(fileName, fontCollection.Get(), fontWeight, fontStyle, fontStretch, fontSize, localeName, &pTextFormat);
//	delete[] fileName;
//	if (FAILED(result)) { return result; }
//
...
//
//	// テキストレイアウトを作成
//	result = pDWriteFactory->CreateTextLayout(wstr.c_str(), static_cast<int>(wstr.size()), pTextFormat.Get(), TargetSize.width, TargetSize.height, pTextLayout.GetAddressOf());
//
//	if (FAILED(result)) { return result; }
//
//	// 描画位置の確定
...
//// options：テキストの整形
//	//====================================
//HRESULT DirectWriteCustomFont::DrawString(std::wstring wstr, D2D1_RECT_F rect, D2D1_DRAW_TEXT_OPTIONS options, bool shadow)
//{
//	HRESULT result = S_OK;
//
//	// 文字列の変換
...
//
//	// 描画処理
//	pRenderTarget->DrawText(wstr.c_str(), static_cast<int>(wstr.size()), pTextFormat.Get(), rect, pBrush.Get(), options);
//
//	// 描画の終了
//	result = pRenderTarget->EndDraw();
//	if (FAILED(result)) { return result; }
